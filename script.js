$(function() {
  ...
  $(".button--start").on("click", function() {
    ...
    $.getJSON("text_db.json", function(json) {
      ...
      for (var i = 0; i < textInputArray.length; i++) {
        textInputArraySave[i] =
          '<span id="x' + i + '">' + textInputArray[i] + "</span>";
  ...
  function typerState() {
    if (iG >= textInputArray.length) {
      console.log("End");
      average(averageSeconds, textInputArray);
      stop = true;
  ...
  function timer(startTimer) {
    ...
    if (startTimer == true) {
      let myInt = setInterval(function() {
        ...
        if (!stop) {
          ...
          time = hourAdd + ":" + minAdd + ":" + secAdd + ":" + msAdd;
          console.log("Your time: " + time + " Errors: " + wrongAnswerCounter);
          clearInterval(myInt);
        }
      }, 10);
    }
    ...
    function refreshTime() {
      let time = hourAdd + ":" + minAdd + ":" + secAdd + ":" + msAdd;
      $(".timer").text(time);
    }
  }

  // calc average words per seconds
  function average(averageSeconds, textInputArray) {
    const av = averageSeconds / 100 / textInputArray.length;
    $(".average").text("Average: " + av.toFixed(2) + " words / second");
  }
